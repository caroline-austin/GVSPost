...
for i = 1:num_csv_files   
    ...
    for j = 1:height(TrialKey)
        ...
        if csv_num == (TrialKey.TTSTrial(j))
            ...
            sgtitle(Trial_name)
            subplot(2,1,1)
            plot(TTS_output.ms(1:end-2), TTS_output.TiltFeedback(1:end-2));
            hold on;
            plot(TTS_output.ms(1:end-2), TTS_output.GVS1MV(1:end-2));
            % plot(TTS_output.ms(1:end-2), TTS_output.GVS2MV(1:end-2));
            plot(TTS_output.ms(1:end-2), TTS_output.GVS3MV(1:end-2));
            legend ("Tilt Angle", "Ch 2",  "Ch3")
            xlabel("time (s)");
            ylabel("Angle (deg) and GVS (mV) ")
             
            subplot(2,1,2)
            plot(TTS_output.ms(1:end-2), TTS_output.AngVel(1:end-2)); hold on;
            plot(TTS_output.ms(1:end-2), TTS_output.GVS1MV(1:end-2));
            % plot(TTS_output.ms(1:end-2), TTS_output.GVS2MV(1:end-2));
            plot(TTS_output.ms(1:end-2), TTS_output.GVS3MV(1:end-2));
            legend ("Tilt Velocity", "Ch 2",  "Ch3")
            xlabel("time (s)");
