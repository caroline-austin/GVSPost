...
% Reads in and combines data from the "master spread sheet"
% (PitchDynamicGVSPlusTilt) in the Data folder, the GVS Profile files that
...
% only the current subject's sheet
    ...
    for i = 1:num_csv_files
       ...
       if ~ contains(current_csv_file, TTS_profile)
            disp(horzcat(["For trial ", trial_number, "and subject", subject_str,"the TTS csv profile does " + ...
       ...
       % the P/N angle coupling trials are flipped here because the GVS
        ...
        elseif contains(GVS_profile_name, '-') && contains(GVS_profile_name, '8') 
            polarity = 'P';
            current_amp = string(abs(str2num(string(Trial_Info(row,5)))));
        elseif ~contains(GVS_profile_name, '-') && contains(GVS_profile_name, '8') 
            polarity = 'N';
       ...
       mustBeNonsparse(GVS_actual2_mV);
       mustBeFinite(GVS_actual2_mV);
       GVS_actual2_filt = lowpass(GVS_actual2_mV,1,50); %filter raw GVS data
