...
Max_MotionRating_map = [All_MotionRating_mapReduced(end,:,:,:) zeros(dim1,1,dim3)];
for i = 1:dim3
    for j = 1:dim4
        check = sum(Sin05Hz_MotionRating_map(:,i,j));
...
for config = 1:num_config %generate electrode subplots
    ...
    MapStackedBarPlot(Sin05Hz_MotionRating_map(2:end,:,config),Title,100, ["none", "noticeable", "moderate", "severe", "no report"],Color_list)
    hold on; 
    set(gca, 'color', [0 0 0]);
    xlim([0.35 4.15]);
...
for config = 1:num_config %generate electrode subplots
    nexttile; 
    Title = strjoin([num2str(config+1) " Electrodes"]);
    MapStackedBarPlot(Sin05Hz_Tingle_map(2:end,:,config),Title,100, ["none", "noticeable", "moderate", "severe", "no report"],Color_list)
    hold on; 
    set(gca, 'color', [0 0 0]);
    xlim([0.35 4.15]);
...
for config = 1:num_config %generate electrode subplots
    nexttile; 
    Title = strjoin([num2str(config+1) " Electrodes"]);
    MapStackedBarPlot(Sin05Hz_Metallic_map(2:end,:,config),Title,100, ["none", "noticeable", "moderate", "severe", "no report"],Color_list)
    hold on; 
    set(gca, 'color', [0 0 0]);
    xlim([0.35 4.15]);
...
for config = 1:num_config %generate electrode subplots
    nexttile; 
    Title = strjoin([num2str(config+1) " Electrodes"]);
    MapStackedBarPlot(Sin05Hz_VisFlash_map(2:end,:,config),Title,100, ["none", "noticeable", "moderate", "severe", "no report"],Color_list)
    hold on; 
    set(gca, 'color', [0 0 0]);
    xlim([0.35 4.15]);
