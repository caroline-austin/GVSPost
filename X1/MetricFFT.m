...
% this script calculates outcome measures (fft, ...) and
...
if ismac || isunix
    plots_path = [file_path '/Plots/Measures/FFT']; % specify where plots are saved
...
% for p = 1: length(prof)
%     eval(["All_avg_fft_" + prof(p) + "= zeros(1,length(match_list));"]);
%     eval(["num_trials_" + prof(p) + "= zeros(1, length(match_list));"]);
%     eval(["num_sub_trials_" + prof(p) + "= zeros(numsub, length(match_list));"]);
%     eval(["fft_save_" + prof(p) + "= zeros(numsub, length(match_list));"]);
% 
%     All_avg_fft_all= zeros(1,length(match_list));
%     All_avg_fft_all_norm= zeros(1,length(match_list));
%     num_trials_all= zeros(1, length(match_list));
%     num_sub_trials_all= zeros(numsub, length(match_list));
%     fft_save_all= zeros(numsub, length(match_list));
%     fft_save_all_norm= zeros(numsub, length(match_list));
% end 
...
% fft calculation for data aggregated by coupling scheme
for j = 1:length(match_list)
     eval(["fft_" + match_list(j) + "= fft(shot_" + match_list(j) + " , 100000)';"])
     eval(["fft_all(:, j)= fft_" + match_list(j) + ";"]);
end
fft_all_norm = fft_all-fft_all(:,4);
% MAE calculation for trials grouped by motion profile
for p = 1: length(prof)
%     eval(["fft_" + prof(p) + "= sum(abs(shot_" + prof(p) ...
...
for p = 1: length(prof)
    subplot(2,3,p)
    eval(["plot_single_outcomes(fft_" + prof(p) + ",Label.shot_" + prof(p) + ", Color_List,match_list);"]);
...
for p = 1: length(prof)
    eval(["[All_avg_fft_" + prof(p) + ",num_trials_" + prof(p) + ...
        ",fft_save_" + prof(p) + ",num_sub_trials_" + prof(p) + ...
        "] =  AggregateSingleMetric(fft_" + prof(p) + ", Label.shot_" ...
        + prof(p) + ", sub, All_avg_fft_" + prof(p) + ",num_trials_" ...
        + prof(p) + ",fft_save_" + prof(p) + ",num_sub_trials_" + prof(p) ...
        + ", match_list);"]);
end

[All_avg_fft_all,num_trials_all ,fft_save_all,num_sub_trials_all] =  AggregateSingleMetric( ...
    fft_all, match_list, sub, All_avg_fft_all,num_trials_all ...
        ,fft_save_all,num_sub_trials_all, match_list);


[All_avg_fft_all_norm,num_trials_all ,fft_save_all_norm,num_sub_trials_all] =  AggregateSingleMetric( ...
    fft_all_norm, match_list, sub, All_avg_fft_all_norm,num_trials_all ...
        ,fft_save_all_norm,num_sub_trials_all, match_list);
%update the label
Label_fft = match_list;

    %% save files
   cd(plots_path);
   vars_2_save = ['Label fft_4A fft_4B fft_5A fft_5B fft_6A fft_6B fft_all fft_all_norm' ];
...
All_avg_fft_all= All_avg_fft_all./num_trials_all;
fft_save_all= fft_save_all./num_sub_trials_all;

All_avg_fft_all_norm= All_avg_fft_all_norm./num_trials_all;
fft_save_all_norm= fft_save_all_norm./num_sub_trials_all;

for p = 1: length(prof)
    eval(["All_avg_fft_" + prof(p) + "= All_avg_fft_" + prof(p) + "./num_trials_" + prof(p) + ";"]);
    eval(["fft_save_" + prof(p) + "= fft_save_" + prof(p) + "./num_sub_trials_" + prof(p) + ";"]);
end 

%create box plot
figure;
b = boxplot(fft_save_all);
% b.BoxFaceColor = blue;
...
for j = 1:numsub
    for i = 1:width(fft_save_all)
        
        plot(i+xoffset2(j), fft_save_all(j, i),sub_symbols(j),'MarkerSize',15,"LineWidth", 1.5);
        hold on;
    end
end
...
%all subjects averaged fft plot (aggregate by coupling scheme)
figure;

    plot_single_outcomes(All_avg_fft_all,Label_fft, Color_List,match_list);
...
figure;
    ...
    plot_single_outcomes(All_avg_fft_all_norm,Label_fft, Color_List,match_list);
...
%all subjects averaged fft plot
figure;
for p = 1: length(prof)
    subplot(2,3,p)
    eval(["plot_single_outcomes(All_avg_fft_" + prof(p) + ",Label_fft, Color_List,match_list);"]);
    title (["Profile " + prof(p)]);
end
    hold on; 
    ...
    hold off; 
%%
    fft_means = mean(fft_save_all, 'omitnan');
    fft_std = std(fft_save_all, 'omitnan');


    %% save files
   cd(plots_path);
   vars_2_save = ['Label_fft fft_save_4A fft_save_4B fft_save_5A fft_save_5B fft_save_6A fft_save_6B fft_save_all fft_save_all_norm' ];
