...
if ismac || isunix
    plots_path = [file_path '/Plots/Measures/Perception-GVS-tiltRemovedSlope']; % specify where plots are saved
...
for j = 1:length(match_list)
    eval(["shot_" + match_list(j) + " = shot_" + match_list(j) + " - tilt_" + match_list(j) + ";"]);
end
...
title(["Tilt Perception V. GVS tiltRemoved Attenuating" subject_str])
xlabel("Current mA")
...
hold off;
cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-Slope-Fit-Negative' datatype subject_str ]); 
    cd(code_path);
    hold off; 
figure;
...
%plot slopes
title(["Tilt Perception V. GVS  tiltRemoved Amplifying" subject_str])
xlabel("Current mA")
ylabel ("Percieved Tilt Angle")
ylim([-20 20]);
legend(match_list(4:end));
cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-Slope-Fit-Positive' datatype subject_str ]); 
    cd(code_path);
    hold off; 
...
for p = 1: length(prof)
    ...
    for trial = 1:col
        ...
        eval(["[LM_" + prof(p) + "." + eval(["Label.shot_" + prof(p) ...
            ...
            "(:,GVS_index), (shot_" + prof(p) + "(:,trial) - tilt_" + prof(p) + ...
            "(:,GVS_index) ), 'Intercept', false);"]); 
...
figure;
    ...
    hold on; 
    title(['Perception-GVS-tiltRemoved-Slope-All-Profiles: Subject ' datatype subject_str]);

    cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-Slope-All-Profiles' datatype subject_str ]); 
    cd(code_path);
    hold off; 
...
for p = 1: length(prof)
    ...
    hold on; 
    sgtitle(['Perception-GVS-tiltRemoved-Slope: Subject ' datatype subject_str]);

    cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-Slope' datatype subject_str ]); 
    cd(code_path);
    hold off;  
...
Label_slope = match_list;
   ...
   eval(['  save ' ['S' subject_str 'Perception-GVS-tiltRemoved-Slope' datatype '.mat '] vars_2_save ' vars_2_save']);      
   cd(code_path)
...
hold on; 
sgtitle(['Perception-GVS-tiltRemoved-Slope-All-Profiles: AllSubjectsBoxPlot' datatype ]);

 cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-Slope-All-ProfilesAllSubjectsBoxPlot' datatype  ]); 
    cd(code_path);
    hold off;
...
hold on; 
sgtitle(['Perception-GVS-tiltRemoved-Slope: AllSubjectsBoxPlot' datatype ]);

 cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-SlopeAllSubjectsBoxPlot' datatype  ]); 
    cd(code_path);
    hold off;
...
figure;
    ...
    hold on; 
    sgtitle(['Perception-GVS-tiltRemoved-Slope-All-Profiles: AllSubjects' datatype ]);

    cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-Slope-All-ProfilesAllSubjects' datatype  ]); 
    cd(code_path);
    hold off; 
...
for p = 1: length(prof)
    ...
    hold on; 
    sgtitle(['Perception-GVS-tiltRemoved-Slope: AllSubjects' datatype ]);

    cd(plots_path);
    saveas(gcf, [ 'Perception-GVS-tiltRemoved-SlopeAllSubjects' datatype  ]); 
    cd(code_path);
    hold off; 
   ...
   eval(['  save ' ['SAllPerception-GVS-tiltRemovedSlope' datatype '.mat '] vars_2_save ' vars_2_save']);      
   cd(code_path)
   eval (['clear ' vars_2_save])
   close all;
