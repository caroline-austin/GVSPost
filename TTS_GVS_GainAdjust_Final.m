...
function avg_gain_rms_min = find_gain(shot,tilt)
    ...
    for trial = 1:num_trials
        ...
        for gain_shift = 1:length(gvec)
            ...
            shot_start = 51+gvec(gain_shift);
            shot_end = num_timesteps-50+gvec(gain_shift);
            tilt_end = num_timesteps -50;

            %calculate and save the error between the actual motion profile and the
            %shot response 
            signal_diff = tilt(51:tilt_end,tilt_index) - shot(shot_start:shot_end,trial)*(gvec(gain_shift));
            gain_rms(gain_shift,trial) = rms(signal_diff); % What does adding one do to indexing?
            %gain_rms(gain_shift + 1) = rms(signal_diff);
        end
    end 
