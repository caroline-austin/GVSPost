...
%TCP = tcpserver(16666);
%%
code_path = '/home/gvslinux/Documents/ChairGVS/Code';
%file_path = '/home/gvslinux/Documents/ChairGVS/Profiles/OscTesting';

%flush(TCP)
clear output
fprintf('Please select your profile to run\n');
cd ..
[filearg,file_path] = uigetfile('*.mat');
fprintf([filearg '\n']);
timeout = 60; %s
...
if port_exist == 1
    ...
    %port.BaudRate = 57600;
    %port.Terminator = []; % no terminator ty
    port.DataBits = 8;
    port.StopBits = 1;
    port.Parity = 'none';
else
...
global index
    if (index <= cmds_arg)
        %fprintf(port_arg,data_arg(index));
        write(port_arg,data_arg(index),'string');
        %fprintf(data_arg(index)); 
        %fprintf('\n'); 
        index = index + 1; 
    end
