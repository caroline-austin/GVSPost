function [bb,dev,stats] = brglmfit(x,y,distr,varargin)
%
% © 2012 Massachusetts Eye and Ear Infirmary.  All rights reserved.

%BRGLMFIT fits a bias-reduced estimate 
%   as described in detail by Chaudhuri and Merfeld in their Experimental 
%   Brain Research paper entitled:
%   "Signal detection theory and vestibular perception: III. Estimating
%   unbiased fit parameters for psychometric functions"
%
%   From a user input/output perspective, this function operates similar 
%   to the MATLAB function that we modified to add the bias-reduction calculations.
%   Therefore, the comments from the MATLAB function GLMFIT are provided
%   below unedited in their entirety.  It is important to note that
%   BRGLMFIT only works with probit and logit models at this time.  If
%   another model is entered an error message is returned.
%
...
%   $Revision: 1.3.2.9 $  $Date: 2008/06/20 09:05:06 $
...
%    estdisp = 'on';
case {'normal','poisson','gamma','inverse gaussian'}
	error('Distribution must be binomial');
otherwise
...
while iter <= iterLim
    ...
    % Bias adjust (changes from original glmfit)--------------
    eta=offset + x*b_old;
    W=diag(sqrtw.^2);
    Q=x*(eye(p)/(x'*W*x))*x';
    Qii=diag(Q);
    ...
    b_correction=(eye(p))/(x'*W*x)*x'*W*zeta;
    b=b-b_correction;
    %---------------------------------------------------------
...
if nargout > 2
    ...
    % statinsertnan is undefined,  since i dont use residuals, just dont
    % assign this variabile to the stats structure.
...
% Link function restrict (changes from original stattestlink for glmfit)
...
%        ilink = @(eta) min(max(eta,lowerBnd),upperBnd) .^ (1/linkExponent);
    case {'identity', 'log', 'comploglog', 'loglog', 'logloglink', 'reciprocal', 'power'}
        error('Link function must be either probit or logit')
    otherwise
