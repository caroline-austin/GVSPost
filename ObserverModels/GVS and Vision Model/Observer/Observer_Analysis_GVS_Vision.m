function [t_s,conf,percepts,others] = Observer_Analysis_Vision(time,motion,varargin)
% Inputs
%
...
model = 'observerModel_GVS_Vision';
sym = 'asymmetric';
...
x_in = motion(:,1:3);
omega_in = motion(:,4:6);
xv_in = motion(:,1:3);%[initializer_array(:,8),initializer_array(:,9),initializer_array(:,10)];
xvdot_in = [initializer_array(:,11),initializer_array(:,12),initializer_array(:,13)];
omegav_in = motion(:,4:6);%[initializer_array(:,14),initializer_array(:,15),initializer_array(:,16)];
gv_in = [initializer_array(:,17),initializer_array(:,18),initializer_array(:,19)];
g_variable_in =ones(length(time),1);
pos_ON =ones(length(time),1);%initializer_array(:,21);%
vel_ON =ones(length(time),1);%initializer_array(:,22);%
angVel_ON =ones(length(time),1);% initializer_array(:,23);
...
% Differentiate Position to Velcoity for visual
xvdot_in = zeros(size(xv_in,1),3);
xvdot_in(1:size(xv_in,1)-1,:) = diff(xv_in,1)/delta_t;
...
% visual parameters to initialize 
% Initialize visual linear position gain
kxvv = 0.1;
% Initialize visual linear velocity gain
kxdotva = 0.75;
% Initialize visual angular velocity gain
kwvw = 10.0;
%Initialize visual tilt angle gain
kgvg = 5;

% Initialize visual position LPF freq
f_visX =2;
%Initialize visual velocity LPF freq
f_visV = 2;
% Initialize visual angular velocity LPF 
f_visO = 0.2;
...
[t_s, XDATA, a_est, gif_est, gif_head, a_head, omega_head,g_head,g_est,...
     omega_est,x_est,lin_vel_est,lin_Vel,x,alpha_omega]...
    = sim(model,duration,options,[]); %#ok<ASGLU>
warning ('on','all');
...
% Store output
conf = 0;%{e_a,e_w,e_f,h};
percepts = {g_est,g_head,omega_head,omega_est,a_head,a_est};
others = 0;%{SCC_GVS_Reg,SCC_GVS_Irreg,alpha_omega,SCC_Physical,...
