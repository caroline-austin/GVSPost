...
function avg_gain = find_gain(shot,tilt)
    ...
    for k = 1:trials
        ...
        posper = shottrial(tilttrial>0);
        postilt = tilttrial(tilttrial>0);
        pi = find(tilttrial>0);

        negper = shottrial(tilttrial<0);
        negtilt = tilttrial(tilttrial<0);
        ni = find(tilttrial<0);

        G = 0.1:0.01:8;
        Cost = zeros(length(G),1);
        for g = 1:length(G)
            overestp = G(g)*posper-postilt;
            overestn = negtilt-G(g)*negper;

            overest = length(overestp(overestp>0))+...
                      length(overestn(overestn>0));
            underest = length(overestp(overestp<0))+...
                       length(overestn(overestn<0));
            
            Cost(g) = abs(overest-underest);
        end
        gain = G(Cost==min(Cost));

        over = zeros(length(tilttrial),1);
        overestp = gain(1)*posper-postilt;
        overestn = negtilt-gain(1)*negper;
        over(pi(overestp>0)) = 1;
        over(ni(overestn>0)) = 1;
        under = zeros(length(tilttrial),1);
        under(pi(overestp<0)) = -1;
        under(ni(overestn<0)) = -1;
        gain_select(k) = gain(1);
